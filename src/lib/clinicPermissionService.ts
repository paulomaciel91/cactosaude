...
export const getClinicEnabledModules = (clinicId: string | number): ClinicEnabledModules => {
  try {
    const stored = localStorage.getItem(STORAGE_KEY);
    if (stored) {
      const allClinicModules = JSON.parse(stored);
      const id = clinicId.toString();
      if (allClinicModules[id]) {
        return allClinicModules[id];
      }
    }
  } catch (e) {
...
export const saveClinicEnabledModules = (clinicId: string | number, modules: ClinicEnabledModules): void => {
  try {
    const stored = localStorage.getItem(STORAGE_KEY);
    let allClinicModules: Record<string, ClinicEnabledModules> = {};
    ...
    const id = clinicId.toString();
    allClinicModules[id] = modules;
    localStorage.setItem(STORAGE_KEY, JSON.stringify(allClinicModules));

    // Disparar evento para atualizar outros componentes
    window.dispatchEvent(new CustomEvent('clinicModulesUpdated', { detail: { clinicId: id } }));
  } catch (e) {
...
export const getCurrentClinicEnabledModules = (): ClinicEnabledModules => {
  try {
    ...
    if (clinicId) {
      return getClinicEnabledModules(clinicId);
    }
  } catch (e) {
...
export const setCurrentClinicId = (clinicId: string | number | null | undefined): void => {
  try {
...
export const isModuleEnabledForClinic = (module: string, clinicId?: string | number): boolean => {
