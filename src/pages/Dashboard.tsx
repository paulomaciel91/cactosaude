...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        setRealStats({
          pacientesAtivos: patientCount || 0,
          consultasHoje: appointmentsToday || 0,
          agendamentosPendentes: pendingCount || 0,
          taxaConclusao: 92 // Mock por enquanto
        });
      } catch (err) {
  ...
  const stats = [
    {
      ...
      value: loading ? "..." : realStats.consultasHoje.toString(),
      change: "+12%",
      icon: Calendar,
    ...
    },
    {
      ...
      value: loading ? "..." : realStats.pacientesAtivos.toString(),
      change: "+8%",
      icon: Users,
    ...
    },
    {
      ...
      value: loading ? "..." : realStats.agendamentosPendentes.toString(),
      change: "-5%",
      icon: CalendarClock,
  ...
  const recentAppointments = [
    { time: "09:00", patient: "Carregando...", type: "Consulta", status: "confirmed" },
  ];
  ...
  const alerts = [
    {
      ...
      message: `${realStats.agendamentosPendentes} pacientes aguardando confirmação`,
      ...
      <div className="flex-1 space-y-4 sm:space-y-6 p-4 sm:p-6">
          ...
          {stats.map((stat, index) => (
            <Tooltip key={index}>
              <TooltipTrigger asChild>
                <Card className={`border-l-4 ${index === 0 ? 'border-l-primary' :
                  ...
                  <CardHeader className="pb-3">
                    <CardTitle className="text-sm font-medium text-muted-foreground flex items-center justify-between">
                      {stat.title}
                      <stat.icon className={`h-4 w-4 ${index === 0 ? 'text-primary' :
                  ...
                  <CardContent>
                    <div className={`text-2xl font-bold ${index === 0 ? 'text-primary' :
                      index === 1 ? 'text-success' :
                        index === 2 ? 'text-warning' :
                          'text-info'
                      }`}>{stat.value}</div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {alerts.map((alert, index) => (
            <Card key={index} className={`border-l-4 ${alert.type === 'warning' ? 'border-l-warning bg-warning/5' :
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full ${alert.type === 'warning' ? 'bg-warning/10' :
                    alert.type === 'error' ? 'bg-destructive/10' :
                      alert.type === 'success' ? 'bg-success/10' :
                        'bg-info/10'
                    }`}>
                    <alert.icon className={`h-5 w-5 ${alert.type === 'warning' ? 'text-warning' :
                  ...
                  <div className="flex-1">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className={`h-8 px-3 transition-all duration-200 ${alert.type === 'warning' ? 'hover:bg-warning/20 hover:text-warning hover:shadow-sm text-warning/80' :
                      ...
                      onClick={() => {
                        if (alert.type === 'info') {
                          ...
                          navigate('/app/agenda?status=pending');
