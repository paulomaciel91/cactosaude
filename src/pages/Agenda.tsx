...
const Agenda = () => {
  ...
  // Efeito para filtrar automaticamente pelo médico logado
  useEffect(() => {
    const userRole = localStorage.getItem('CactoSaude_user_role');
    const userName = localStorage.getItem('cactosaude_user_name');
    if (userRole === 'medico' && userName) {
      setFilterProfessional(userName);
    }
  }, []);
  ...
  // Lista de profissionais sincronizada com o banco de dados
  ...
  useEffect(() => {
    const fetchProfessionals = async () => {
      const clinicId = localStorage.getItem('cactosaude_current_clinic_id');
      ...
      if (!error && data && data.length > 0) {
        setProfessionals(data.map(p => p.full_name || 'Profissional'));
      }
    };
    fetchProfessionals();
  }, []);
  ...
  useEffect(() => {
    ...
    loadPatients();
    const unsubscribe = patientService.onPatientsChange(loadPatients);
    return unsubscribe;
  }, []);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (clinicId && clinicId !== '00000000-0000-0000-0000-000000000000') {
        console.log('Agenda: Buscando dados do Supabase para clínica:', clinicId);
        await appointmentService.fetchAppointments(clinicId);
        await appointmentService.fetchBlockedSlots(clinicId);
      }
      ...
      // ... resto do log omitido por brevidade
    };
  ...
  const handleBlockTime = () => {
    ...
    const newBlock: BlockedSlot = {
      ...
      professional_name: blockProfessional,
      clinic_id: localStorage.getItem('cactosaude_current_clinic_id') || undefined,
      reason: blockReason,
    };
        ...
        <div className="grid grid-cols-7 gap-1 sm:gap-2">
          ...
          {days.map((dayData, i) => {
            ...
            return (
              <div
                key={i}
                className={`border rounded-lg p-1 sm:p-2 min-h-[60px] sm:min-h-[80px] lg:min-h-[100px] transition-colors ${isToday ? 'bg-primary/10 border-primary' : ''
                        ...
                        {appointmentsInSlot.map((appointment, aptIndex) => {
                          ...
                          return (
                            <Tooltip key={`tooltip-apt-${appointment.id}`}>
                              <TooltipTrigger asChild>
                                <div
                                  className={`absolute rounded cursor-grab active:cursor-grabbing hover:opacity-90 transition-all z-[10] ${draggedAppointment?.id === appointment.id ? 'opacity-50' : 'opacity-100'
                                    }`}
                                  draggable={true}
                                  onMouseEnter={(e) => e.stopPropagation()}
                                  onMouseLeave={(e) => e.stopPropagation()}
                ...
                <div className="space-y-2 pr-4">
                  {filteredAppointments.length === 0 ? (
                    ...
                    filteredAppointments.map((appointment) => (
                      <div
                        key={appointment.id}
                        className={`p-3 sm:p-4 rounded-lg border transition-colors cursor-pointer ${selectedCardId === appointment.id
